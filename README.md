...
main
  ...
  -> 登录页显示后：展示“请同意《隐私协议》和《用户协议》”复选框，不自动弹窗
  -> 未勾选并点击登录：覆盖协议弹窗；同意后选中复选框，拒绝后保持未选中
...
- 本地没有任何同意版本：这是首次授权。先清除安全存储中可能残留的旧会话，再创建 `MyApp` 并直接显示
  登录页。页面展示协议复选框但不自动弹窗；未勾选并点击登录时，`PrivacyConsentHost` 才通过根
  `DialogRoute` 显示协议说明。同意并成功保存后选中复选框；账号和密码都已填写时继续本次登录，缺少任一
  字段时只选中、不弹 Toast。不同意只关闭弹窗、取消勾选并停留登录页。
...
越过授权发送；下次冷启动仍是未选中状态，但也不会自动打断用户输入。已经同意当前版本后，后续启动仍不把
...
首次登录不会自动弹窗。默认 `LoginPage` 在未勾选且点击登录时向 App 级 `PrivacyConsentHost` 请求唯一弹窗；
项目替换成

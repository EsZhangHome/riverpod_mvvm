...
启动流程分为五段。最重要的是分清：Bootstrap 不负责登录，`StarterHomePage` 也不是
启动页。正常冷启动的真实顺序如下：
...
main
  ...
  -> PrivacyConsentGate：恢复并核对已同意政策版本
  -> 从未同意：清除可能残留的安全会话，再创建 MyApp 并直接进入登录页
  -> 登录页显示后：展示复选框，并自动覆盖一次首次协议弹窗
  -> 首次拒绝：关闭弹窗、保持未选中，本次运行不再自动弹
  -> 之后未勾选并点击登录：再次覆盖协议弹窗
  -> 版本过期：创建 MyApp，并在当前页面上覆盖政策升级弹窗
  -> 已同意当前版本：正常创建 MyApp
  -> MyApp / GoRouter
...
环境安全校验
  ...
  -> 创建 PrivacyConsentGate
```

为什么内层业务 `ProviderScope` 不直接写在 `main.dart`？`themeProvider` 首次构建会同步读取普通存储，
隐私版本、旧版会话迁移和主题都需要读取普通偏好。Bootstrap 先尝试初始化 `LocalStorage`，再创建业务
Provider，才能让这些同步恢复拿到可靠结果。项目通过 `rootBuilder` 提供的外层 ProviderScope 可以提前存在，因为
...
### 第三段：恢复隐私记录，区分首次登录和政策升级

`PrivacyConsentGate` 优先读取 `privacy_consent_record_v1`，旧安装仍兼容
`privacy_policy_accepted_version`。结构化记录包含授权版本、隐私政策正文版本、用户协议正文版本和 UTC
同意时间；门禁用
`ENV_PRIVACY_POLICY_VERSION`（授权版本）判断是否需要重新确认：

- 本地没有任何同意版本：这是首次授权。先清除安全存储中可能残留的旧会话，再创建 `MyApp` 并直接显示
  登录页。认证恢复确认未登录后，`PrivacyConsentHost` 通过根 `DialogRoute` 自动显示一次协议说明。同意并
  成功保存后选中复选框，不校验空表单；不同意只关闭弹窗、保持未选中并停留登录页，本次运行不再自动弹。
  之后未勾选点击登录会再次显示同一个弹窗：这时同意后若账号密码完整则续接本次登录，缺少任一字段时只
  选中、不弹 Toast。
- 本地有旧版本：这是政策升级。正常保留 `MyApp` 和当前路由，在整个 Navigator 上方显示不可绕过的升级弹窗。
  同意且保存成功后弹窗消失，用户仍停留在原页面；不同意时弹窗会一直遮挡页面，等退出登录和安全会话清理
  完成后才关闭。安全存储清理会自动重试一次；如果 Keychain/Keystore 仍不可用，弹窗显示可重试错误并继续
  遮挡，不能把“内存已退出”误报成“持久会话已清除”。旧版本保留到本地，因此
  当前进程不会反复弹出，下次启动仍会再次提醒。

拒绝不会保存任何同意记录。只要复选框没有选中，每次点击登录都会重新请求同一个协议弹窗，登录接口不会
越过授权发送；下次冷启动会再次自动显示首次提示。已经同意当前版本后，后续启动仍不把
历史记录自动映射成“本页已勾选”，用户需要在本次登录界面明确选择；历史记录负责合规版本判断和 Warmup
放行，页面复选框负责本次登录交互，两者职责不同。

首次登录与政策升级不会出现两个弹窗：状态恢复时三种结果互斥——没有历史版本是首次授权、有旧版本是政策
升级、版本一致是已授权。`PrivacyPromptCoordinator` 只管理弹窗请求互斥与登录等待结果，
`PrivacyConsentHost` 是唯一调用 Flutter DialogRoute 的 Presenter；默认登录、SSO、登录触发提示和升级提示
不会各自创建 Dialog。全局升级弹窗存在时登录检查只返回 false，不会再叠加第二层弹窗。

`ENV_PRIVACY_POLICY_DOCUMENT_VERSION` 和 `ENV_USER_AGREEMENT_DOCUMENT_VERSION` 分别表示用户实际阅读的
两份正文版本，错别字、排版等普通修订只升级正文版本，不会
强制重复同意；数据处理目的、范围或共享对象发生实质变化时，才升级 `ENV_PRIVACY_POLICY_VERSION`。弹窗中的
“阅读完整隐私政策”和“阅读完整用户协议”会使用 `url_launcher` 打开各自经过环境校验的 HTTPS 地址，不再叠
一层只显示 URL 的占位弹窗。
接入真实项目时必须同步修改四项显著告知文案、完整政策正文和 SDK/权限清单，底座示例文案不能代替法务审核。

底座还注册了公开路由 `/privacy-center`。默认 Starter 首页提供入口；接入真实业务后，在项目自己的“设置”或
“关于”页面继续导航到 `RoutePaths.privacyCenter` 即可。该页面直接读取同一个 `privacyConsentProvider`，展示
当前/历史版本、同意时间、四项显著告知和两份完整协议，不会再保存第二份布尔状态。已存在历史授权时还会提供
“撤回授权并退出登录”：Privacy Feature 先立即停止内存放行并删除授权记录，App 组合层再严格清理安全会话，
全部成功后才回到项目自己的 `loginPath`；失败会保留可重试提示。
...
首次授权前允许创建登录页所需的 `MyApp`、路由和认证状态，但不会发送登录请求，也不会执行需要当前政策同意
的两个 AppWarmup 阶段。这样既能让用户先看到登录界面，又不会因为页面已经出现就提前初始化统计、推送等
敏感能力。政策升级时也采用同一条 Warmup 门禁。

首次无记录时先清理安全会话，是为了处理普通偏好已被清除、但 iOS Keychain 或 Android 安全存储仍残留旧
token 的边界情况。清理失败会显示可重试错误页，不会冒险恢复旧账号。隐私拒绝不再尝试杀死 Android/iOS
进程，因此 MainActivity 和 AppDelegate 不需要维护隐私退出通道。

退出登录不会删除隐私同意版本；卸载、清除应用数据或主动撤回会回到首次授权状态，政策版本变化会进入升级
状态。
但三方 SDK 如果通过 Android ContentProvider、Application 或 iOS AppDelegate 自动初始化，仍需按 SDK
文档关闭自动初始化，改为同意后手动启动。

### 第四段：恢复安全会话，再决定登录页或业务页
...
同一阶段只执行一次，阶段内任务并行；单个任务失败只记录问题，不会盖住登录页或首页。项目覆盖
`appWarmupTasksProvider` 时，即使任务注册表的组装代码本身抛错，也会生成 `registry.<phase>` 问题并结束
加载状态，不会让 Warmup 永久停在 `AsyncLoading`。
两个阶段还共同要求“已同意当前隐私政策版本”：政策升级弹窗未处理或已拒绝时，尚未启动的 Warmup 不会运行。
若某个 SDK 已在原生层自动启动，仍需关闭自动初始化或调用厂商提供的停止采集接口。

真实项目接入需要授权的统计、崩溃或推送 SDK 时，再使用 `PrivacyProtectedSdkInitializer` 包住对应 Adapter。
它会在调用 Adapter 前重新读取 Riverpod 当前状态，旧版本、未同意和保存失败都不会触碰 SDK；并发调用复用
同一个 Future，成功后也不会重复初始化。`PrivacyConsentProof` 只能由门禁创建，原生 Adapter 应把其中的授权
版本传给 Android/iOS，再由原生侧与当前构建要求的版本进行第二次比较后调用厂商 SDK：

```dart
final analyticsInitializerProvider = Provider((ref) {
  return PrivacyProtectedSdkInitializer(
    readConsentState: () => ref.read(privacyConsentProvider),
    adapter: ProjectAnalyticsAdapter(),
  );
});
...
final appWarmupTasksProvider = Provider<List<AppWarmupTask>>((ref) {
  return [
    AppWarmupTask(
      name: 'analytics',
      phase: AppWarmupPhase.afterSessionReady,
      run: () async {
        await ref.read(analyticsInitializerProvider).initializeIfAllowed();
      },
    ),
  ];
});
...
`ProjectAnalyticsAdapter.initializeWithConsent(proof)` 才能 import 厂商 SDK 或调用平台桥。Android
`Application`/`ContentProvider`、iOS `AppDelegate` 中的自动初始化必须关闭；否则 Dart 门禁再完整也无法阻止
原生代码在用户同意前采集。底座没有预置一个空 MethodChannel，因为不同厂商的初始化参数和停止采集 API
不同，只有真实项目 Adapter 才能正确完成第二层校验。
...
| 首次无记录时清理残留会话 | `PrivacyConsentGate` |
| 所有隐私弹窗唯一的 DialogRoute Presenter | `PrivacyConsentHost` |
| 登录重试与弹窗互斥、结果回传 | `PrivacyPromptCoordinator` |
| 默认/自定义登录的提交前门禁 | `requestPrivacyConsentBeforeLogin` |
| 登录前后随时查看、撤回授权 | `/privacy-center`、`PrivacyCenterPage` |
| 需要授权的三方 SDK 二次门禁 | `PrivacyProtectedSdkInitializer` |
...
恢复、登录写入、Token 刷新写入和退出清理共享同一条安全存储队列，并使用会话版本号判断异步结果是否仍然
有效。例如刷新接口尚未返回时用户点击退出，迟到的新 Token 会被丢弃，不能把用户重新变成已登录；启动恢复
失败期间若用户建立了新会话，旧恢复结果也不能覆盖新登录。普通退出会立即清内存状态，并在后台重试一次
持久清理；隐私升级拒绝使用严格模式，持久清理失败必须反馈给全局弹窗。
...
首次无记录时，App 级 `PrivacyConsentHost` 会等认证恢复确认进入登录页后自动显示一次；拒绝后本次运行不再
自动重复，但默认 `LoginPage` 在未勾选且点击登录时仍会请求同一个弹窗。项目替换成
短信、SSO 或品牌登录页后，需要在自定义提交动作中显式保留同一检查：

```dart
loginBuilder: (context, state) => SsoLoginPage(
  onSubmit: (agreementSelected) async {
    final canLogin = await requestPrivacyConsentBeforeLogin(
      context,
      agreementSelected: agreementSelected,
    );
    if (!canLogin) return; // 用户不同意：弹窗已关闭，仍停留在登录页。
    await submitSsoLogin();
  },
),
```

不要在构建页面时调用它，也不要在弹窗同意前先发登录接口。函数本身不会创建 Dialog，只向 App 级
`PrivacyPromptCoordinator` 发出请求并等待唯一 Presenter 的结果；返回 `true` 后再提交，才能保证不同意
不会产生任何账号或设备数据请求。

...
lib/
  ...
  features/    # 业务模块：login、main、home、orders、mine、learning 等
```
...
`StatefulShellRoute` 保证三个 Tab 共享同一个 `MainPage` 实例，切换 Tab 不销毁子页面并保留滚动位置。Provider 数据是否保留仍由自身生命周期决定，不能把 indexed stack 当成 Provider 缓存策略。
...
- `RiverpodLearningPanel`：学习中心的场景、API、数据流和代码入口说明卡
...
- [test/features/mine/global_service_providers_test.dart](test/features/mine/global_service_providers_test.dart)

...
| --- | --- | --- | --- |
| `flutter_riverpod` `3.3.2` | 运行依赖 | 全局状态管理 + 依赖注入，替代 Provider + get_it | `app.dart` 中通过 `ProviderScope` 提供；各层通过 `ref.watch/read` 获取 |
...
- 统一继承 `Notifier<State>`；页面级自动释放通过 `NotifierProvider.autoDispose` 声明，不再使用 Riverpod 2 的 `AutoDisposeNotifier` 接口。
- Provider 初始化抛出异常时，Riverpod 3 默认会执行指数退避重试。当前 ViewModel 的网络请求由显式方法和 Dio 重试策略管理，不在 `build()` 中抛出网络异常，避免双重重试。
...
每个页面级 ViewModel 使用 `NotifierProvider.autoDispose`，在 `build()` 中创建一个 `AsyncRequestHandler` 实例，并通过 `ref.onDispose` 释放。Riverpod 3 的页面生命周期、请求取消链路如下：
...
依赖注入使用 Riverpod 的 `Provider<T>` 和 `NotifierProvider`，替代了旧的 `get_it`。Repository 的依赖必须由 Provider 显式组装，不允许在 Repository 内回退到 `ApiClient.instance` 等全局单例。

当前注册了：

- `services.dart`：`apiClientProvider`、`apiServiceProvider`、`databaseServiceProvider`、`networkStatusServiceProvider`、`permissionServiceProvider`、`appInfoServiceProvider`
- `repositories.dart`：`homeBannerCacheProvider`、`homeRepositoryProvider`、`loginRepositoryProvider`、`profileRepositoryProvider`

各 ViewModel 通过独立的 `NotifierProvider` 提供（定义在各自的 `view_model.dart` 文件中）。App 级状态使用普通 `NotifierProvider`；路由页面状态使用 `NotifierProvider.autoDispose`。
...
final orderProvider =
    NotifierProvider.autoDispose<OrderNotifier, OrderState>(
      OrderNotifier.new,
    );
```
...
当前全量测试共 24 个，其中生命周期测试会验证关闭页面 Provider 的最后一个监听者后，正在进行的请求会收到取消信号。

...
// 5. 点击订单打开 family 详情和 family 实时物流；关闭弹窗后物流订阅释放，
//    已成功的详情按 ViewModel 配置继续保留一段 TTL。
...
class OrdersPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text(DemoStrings.ordersTitle),
        actions: [
          IconButton(
            tooltip: DemoStrings.reloadOrders,
      ...
      floatingActionButton: FloatingActionButton.extended(
        ...
        label: const Text(DemoStrings.createOrder),
      ),
        ...
        child: feed.when(
          ...
          error: (error, _) => ListView(
            children: [
              ...
              const Center(child: Text(DemoStrings.ordersLoadFailed)),
              TextButton(
                onPressed: () => ref.invalidate(orderFeedProvider),
                child: const Text(DemoStrings.retry),
              ),
            ],
          ),
          data: (state) => ListView(
            ...
            children: [
              const Card(
                child: Padding(
                  ...
                  child: Text(DemoStrings.ordersSceneDescription),
                ),
              ),
              ...
              SegmentedButton<OrderFilter>(
                segments: const [
                  ButtonSegment(
                    ...
                    label: Text(DemoStrings.all),
                  ),
                  ButtonSegment(
                    value: OrderFilter.active,
                    label: Text(DemoStrings.activeOrders),
                  ),
                  ButtonSegment(
                    value: OrderFilter.finished,
                    label: Text(DemoStrings.finishedOrders),
                  ),
                ],
                selected: {filter},
              ...
              if (orders.isEmpty)
                const Center(child: Text(DemoStrings.noFilteredOrders))
              else
              ...
              if (state.hasMore)
                OutlinedButton(
                  ...
                  child: state.isLoadingMore
                      ...
                      : const Text(DemoStrings.loadNextPage),
                )
              else
                const Center(child: Text(DemoStrings.noMoreOrders)),
      ...
      child: ListTile(
        ...
        trailing: isUpdating
            ...
            ? TextButton(
                ...
                child: const Text(DemoStrings.cancel),
              )
...
class _OrderDetailDialog extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return AlertDialog(
      title: const Text(DemoStrings.orderDetail),
      content: detail.when(
        loading: () => const SizedBox(
          height: 100,
          child: Center(child: CircularProgressIndicator()),
        ),
        error: (error, _) => const Text(DemoStrings.orderDetailLoadFailed),
        data: (order) => Column(
          ...
          children: [
            Text(order.title),
            const SizedBox(height: AppSpacing.md),
            const Text(DemoStrings.orderDetailCacheDescription),
            const SizedBox(height: AppSpacing.md),
            Text(
              liveStatus.when(
                loading: () => DemoStrings.connectingLogistics,
                error: (error, _) => DemoStrings.logisticsConnectionFailed,
                data: (status) =>
                    DemoStrings.liveLogistics(_statusLabel(status)),
              ),
            ),
          ],
        ),
      ...
      actions: [
        TextButton(
          ...
          child: const Text(DemoStrings.close),
        ),
      ],
    );
  }
...
String _statusLabel(OrderStatus status) => switch (status) {
  OrderStatus.pendingPayment => DemoStrings.pendingPayment,
  OrderStatus.processing => DemoStrings.processing,
  OrderStatus.shipped => DemoStrings.shipped,
  OrderStatus.delivered => DemoStrings.delivered,
  OrderStatus.cancelled => DemoStrings.cancelled,
};

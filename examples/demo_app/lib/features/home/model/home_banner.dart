...
//
// 放在 demo/features/home/model/ 的原因：
// HomeBanner 是首页模块专用的数据模型，其他模块不会用到，
// 所以由 demo/features/home 持有，不提升为无业务归属的 shared 类型。
//
// 设计要点：
...
/// 首页 Banner 数据模型。
///
/// 页面已经按 imageUrl 字段渲染；接入真实图片时通常只需关闭 Mock，并按后端协议
/// 调整 DemoEndpoints 与 HomeRepository 的解码逻辑。如果真实协议字段或展示需求
/// 不同，Model 和 View 仍应随业务要求正常修改，而不是被这份示例限制。
@JsonSerializable()
class HomeBanner {
  const HomeBanner({
    required this.id,

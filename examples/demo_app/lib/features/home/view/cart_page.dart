...
class CartPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text(DemoStrings.cartTitle),
        actions: [
          if (items.isNotEmpty)
            IconButton(
              tooltip: DemoStrings.clearCart,
  ...
  Future<void> _confirmClear(BuildContext context, WidgetRef ref) async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        title: const Text(DemoStrings.clearCartConfirmTitle),
        content: const Text(DemoStrings.clearCartConfirmMessage),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context, false),
            child: const Text(DemoStrings.keepCart),
          ),
          FilledButton(
            onPressed: () => Navigator.pop(context, true),
            child: const Text(DemoStrings.confirmClearCart),
          ),
        ],
      ),
    );
          ...
          children: [
            Row(
              children: [
                ...
                IconButton(
                  tooltip: DemoStrings.removeCartItem,
            ...
            Text(DemoStrings.cartUnitPrice(product.price.toStringAsFixed(2))),
            const SizedBox(height: AppSpacing.xs),
            Text(
              DemoStrings.cartSubtotal(item.subtotal.toStringAsFixed(2)),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              DemoStrings.cartEmpty,
              style: Theme.of(context).textTheme.titleLarge,
            ),
            const SizedBox(height: AppSpacing.sm),
            const Text(DemoStrings.cartEmptyDescription),
            const SizedBox(height: AppSpacing.lg),
            FilledButton.tonal(
              // 正常从首页 push 进来时 pop；深链进入时回到商品根路由。
              onPressed: () => context.canPop()
                  ? context.pop()
                  : context.go(DemoRoutePaths.mainHome),
              child: const Text(DemoStrings.continueShopping),
            ),
          ],
        ),
      ),

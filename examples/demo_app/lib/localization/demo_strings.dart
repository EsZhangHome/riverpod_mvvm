// lib/localization/demo_strings.dart
//
// 作用：集中管理 Demo 页面复用的固定 UI 文案。
//
// 设计要点：
// 1. 固定文案使用 static const；带占位参数的文案使用静态格式化方法
// 2. 企业外壳已使用 ARB；这里保留中文示例业务文案，便于集中阅读案例
// 3. 集中管理的好处：通用 UI 文案可从一个入口修改；Mock 业务数据仍归 Repository
// 4. 分类清晰：导航类、操作类、错误提示类、Mock 提示类
//
// 新增正式业务时，应把用户可见文案放入 lib/l10n/*.arb，并通过
// AppLocalizations.of(context) 读取；案例迁移时可按 feature 分批替换。

/// Demo 教学文案集中管理。
///
/// 这些常量属于中文教学 Demo，不作为新业务的国际化写法示范。
/// 不要在页面中直接写中文字符串。
class DemoStrings {
  const DemoStrings._();
  ...
  static const String basicLearningCodeEntry =
      'demo/features/home/model → repository → view_model/catalog_view_model.dart → view/home_page.dart';
  ...
  static const String asyncLearningCodeEntry =
      'demo/features/orders/model → repository → view_model/order_view_model.dart → view/orders_page.dart';

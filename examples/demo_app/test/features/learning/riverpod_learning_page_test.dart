...
import 'package:riverpod_mvvm_demo/features/learning/model/riverpod_lesson.dart';
import 'package:riverpod_mvvm_demo/features/learning/view/riverpod_learning_page.dart';
...
void main() {
  testWidgets('独立学习中心在窄屏切换基础、异步和全局内容', (tester) async {
    ...
    await tester.tap(
      find.descendant(
        of: stageSelector,
        matching: find.text(DemoStrings.learningAsync),
      ),
    );
    await tester.pumpAndSettle();
    expect(find.text(DemoStrings.asyncLearningScene), findsOneWidget);

    // Act 2：继续切到全局阶段，并检查最后一次构建没有布局异常。
    await tester.tap(
      find.descendant(
        of: stageSelector,
        matching: find.text(DemoStrings.learningGlobal),
      ),
    );
    await tester.pumpAndSettle();
    expect(find.text(DemoStrings.globalLearningScene), findsOneWidget);

...
import 'package:riverpod_mvvm_demo/features/learning/view/widgets/riverpod_learning_panel.dart';
...
void main() {
  testWidgets('学习面板按统一结构展示路线和当前阶段', (tester) async {
    ...
    expect(find.text(DemoStrings.learningBasic), findsOneWidget);
    expect(find.text(DemoStrings.learningAsync), findsOneWidget);
    expect(find.text(DemoStrings.learningGlobal), findsOneWidget);
    expect(find.text('第 2 站 · 异步'), findsOneWidget);
    expect(find.text(DemoStrings.learningScene), findsOneWidget);
    expect(find.text(DemoStrings.learningApis), findsOneWidget);
    expect(find.text(DemoStrings.learningDataFlow), findsOneWidget);
    expect(find.text(DemoStrings.learningInteraction), findsOneWidget);
    expect(find.text(DemoStrings.learningCodeEntry), findsOneWidget);

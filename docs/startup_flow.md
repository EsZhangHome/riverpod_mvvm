...
登录页显示完成 -> 显示协议复选框，并通过唯一 PrivacyConsentHost 自动弹一次
首次同意成功   -> 保存两份正文版本和 UTC 时间并选中，不校验空表单
首次拒绝       -> 不保存版本、保持未选中并关闭，本次运行不再自动弹
未勾选点登录   -> 再次显示；同意后输入完整则续接登录，否则静默停留
...
Dialog。首次拒绝不保存记录，本次运行不重复自动弹；再次点击登录仍会拦截，下次冷启动再次自动提示。升级拒绝保留旧版本，下次启动也会
...
`/privacy-center` 是独立于授权 Dialog 的公开页面：登录前后都可以查看当前版本、历史记录和完整协议。页面
没有第二份授权状态；撤回时仍调用同一个 PrivacyConsentNotifier，再由 MyApp 组合 Auth 严格退出登录。因此
增加设置页入口不会产生第二套弹窗、布尔值或会话清理逻辑。
...
会话恢复、登录写入、Token 刷新写入和退出清理会按命令顺序经过同一条安全存储队列。每个异步命令还会捕获
一个会话版本号：等待期间发生了更新的登录或退出时，旧命令的结果会被丢弃。因此“刷新返回晚于退出”不会
重新登录，“损坏会话恢复失败晚于新登录”也不会覆盖新会话。政策升级拒绝会要求安全存储清理真正成功；两次
清理都失败时弹窗保持并提示重试，而不是只清内存后假装完成。
...
需要隐私授权的原生 SDK 还应通过 PrivacyProtectedSdkInitializer 调用项目 Adapter：Dart 层先核对当前版本，
Adapter 再把不可伪造的 PrivacyConsentProof 交给 Android/iOS 做第二次版本校验。厂商自动初始化必须关闭。

...
- Dependabot、CODEOWNERS、PR/Issue 模板、安全报告入口、依赖许可证清单和手动 Release Workflow 示例。
...
5. 在 `main.dart` 把 `createStarterRouteBundle()` 换成项目路由包，并移除
   `rootBuilder: buildStarterRoot` 或换成项目自己的依赖组合；不要修改通用 `AppRouter`。
6. 编译通过后删除 `lib/app/starter/` 和它自己的 `test/app/starter/`；开发 Mock 会随组件一起删除，其他
   底座文件与通用测试无需清理引用。
...
`flutter gen-l10n` 会把缺少翻译的 key 写入根目录 `l10n_untranslated.json`；检查工具发现任何非空项都会
返回失败。不要通过删除报告或跳过 CI 解决遗漏，应补齐各语言 ARB 后重新生成。
...
仓库提供的 `.github/workflows/release.yml` 只允许手动触发，默认只构建和上传经过校验的 Android APK/AAB，
不会因为 push 或 tag 自动发布。接入前阅读 [手动 Release Workflow 使用说明](release_workflow.md)，配置签名
Secrets、受保护的 production Environment 和真实生产配置；iOS 发布流程需要按组织证书另行接入。
...
2. 生成本地化代码，检查 `l10n_untranslated.json` 没有遗漏翻译且生成文件无差异。
3. Dart 格式校验。
...
Dependabot 每周检查 pub 和 GitHub Actions 更新；major 依赖不会与 minor/patch 混成一个 PR。PR/Issue、
Security Policy 和 CODEOWNERS 只是可替换模板，新项目必须更新仓库地址、负责人和安全响应规则。

团队可在此基础上提高覆盖率阈值，增加 Sonar、依赖漏洞扫描和公司分发平台。签名 Release 示例已经提供，
但发布渠道、iOS 签名和组织密钥不会硬编码进通用底座。
